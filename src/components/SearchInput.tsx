...
const SearchInput: React.FC<Props> = ({ value, onChange, onSubmit, placeholder='Поиск по заголовку', className }) => {
  return (
    <form onSubmit={(e) => {
        ...
        <button type='submit' className='px-4 py-2 bg-[#5031be] text-white rounded-lg hover:bg-[#3f1dbb] transition-colors cursor-pointer mr-1' aria-label='Искать'>Поиск</button>
    </form>
  );
};
